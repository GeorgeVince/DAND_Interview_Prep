...
109 but 101-107is not a 6-gap because there is 103in between and 103-109 is not
a 6-gap because there is 107 in between.
"""
...
def gap(g, m, n):
    ...
    for i in range(m, (n - g) + 1):
        if isprime(i) and isprime(i + g):
            for ii in range(i + 1, (i + g) - 1):
                if isprime(ii):
                    extra_prime = True
            if extra_prime == False:
                return [i, i + g]
